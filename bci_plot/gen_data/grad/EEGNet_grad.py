...
class DepthwiseConv2d(nn.Conv2d):
    ...
    def forward_from_expanded(self, input):
        return F.conv2d(input, self._max_norm(self.weight), self.bias, self.stride,
                        self.padding, self.dilation, self.D*self.groups)
    def expand_input(self, input):
        out = torch.repeat_interleave(input, self.D, dim=-3)
        return out
...
class EEGNet(nn.Module):
    ...
    def forward(self, x, return_logits=True, return_dataclass=False):
        ...
        X = self.reshape_input(x) # (batch, 1, n_electrodes, T)

        # Block 1
        # -------
        X = self._conv1(X) # (batch, F1, C, T)
        ha = X
        ha.retain_grad()
        ...
        X = self._depthwise.expand_input(X) # (batch, F1*D, C, T)
        
        hb = X # here is same
        hb.retain_grad()
        
        hc = torch.ones((X.shape[1], X.shape[2]), device=X.device, requires_grad=True)
        X = X*hc[None, :, :, None]
        X = self._depthwise.forward_from_expanded(X) # (batch, F1*D, 1, T)
        #X = self._depthwise(X) # (batch, F1*D, 1, T)
        #hb = X # here is not same.
        #hb.retain_grad()
        ...
        if return_dataclass == 'extended':
            return (probs, logits, hidden_state, ha, hb, hc)
        if return_dataclass:
